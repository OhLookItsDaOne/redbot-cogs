...
class AIHelp(commands.Cog):
    ...
    **Commands:**
    ...
    | **!aihelp**                                                        | Generates an AI response with RAG context (accessible to everyone).    |
    | **!aihelpconfig addsource <url> [category] [tags]**                  | Downloads and adds a source file. Category defaults to "general".      |
    | **!aihelpconfig listsources [<category>]**                           | Lists source files (shows file names per category).                    |
    | **!aihelpconfig removesource <category> <filename>**                 | Removes a source file from a category.                               |
    | **!aihelpconfig updatesourcetags <category> <filename> <tags>**        | Updates the tags (comma‑separated) for a source file.                 |
    | **!aihelpconfig movesource <old_category> <filename> <new_category>**  | Moves a source file from one category to another.                    |
    | **!aihelpconfig setchannels <channel ids>**                          | Sets proactive support channels (Owner only).                        |
    | **!aihelpconfig reloadsources**                                      | Reloads all sources from disk.                                         |
    | **!aihelprate <category> <filename> <up|down>**                      | Rates a source as helpful (up) or not (down).                          |
    ...
    async def aihelpconfig_addsource(self, ctx, url: str, category: str = "general", *, tags: str = ""):
        """
        Downloads a source file from a URL and saves it in the specified category.
        ...
        # Verwende den letzten URL-Teil als Dateiname.
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        # Aktualisiere Metadata: Relativer Pfad im Format "Kategorie/filename"
