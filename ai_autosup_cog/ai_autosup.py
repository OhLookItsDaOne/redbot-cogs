...
import aiohttp, io, re, os, json, asyncio, sys, subprocess, logging
import httpx
...
class AIHelp(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # Hier wird der OpenAI-Client (AsyncOpenAI) initialisiert, sobald er benötigt wird.
        self.openai_client = None

    # Helper-Methoden für Berechtigungsprüfungen
    async def is_owner(self, ctx):
        return await ctx.bot.is_owner(ctx.author)

    async def is_owner_or_admin(self, ctx):
    ...
    async def initialize_openai_client(self, ctx: commands.Context = None):
        """
        Initializes the OpenAI client (AsyncOpenAI) using the configured API endpoint
        and the API key from shared tokens.
        """
        endpoint = await self.config.api_endpoint()
        if not endpoint:
            if ctx:
                await ctx.send("API endpoint not configured. Use `!aihelpowner endpoint <url>`.")
            else:
                logger.error("API endpoint not configured.")
            return

        # Hol den OpenAI API Key aus den Shared API Tokens
        api_key = (await self.bot.get_shared_api_tokens("openai")).get("api_key")
        if not api_key:
            if ctx:
                await ctx.send("OpenAI API key not set. Use `!set api openai api_key,<your_api_key>`.")
            else:
    ...
    async def call_api(self, prompt: str, model: str = None) -> str:
        """
        Calls the API using the chat completions endpoint.
        If no model is provided, the default_model from the configuration is used.
        """
        ...
        # Falls kein Modell explizit übergeben wurde, verwende das in der Konfiguration gespeicherte Standardmodell.
        if model is None:
            model = await self.config.default_model()

        try:
            response = await self.openai_client.chat_completions.create(
                model=model,
    ...
    async def aihelpowner_endpoint(self, ctx, endpoint: str):
        ...
        Example: `!aihelpowner endpoint https://api.openai.com/v1/chat/completions`
        """
        if not (await self.is_owner(ctx)):
            await ctx.send("You do not have permission to use this command.")
            return
    ...
    async def aihelpconfig(self, ctx):
        ...
        Subcommands:
         ...
         - reloadsources
        """
        if not (await self.is_owner_or_admin(ctx)):
            await ctx.send("You do not have permission to use this command.")
            return
        if ctx.invoked_subcommand is None:
    ...
    async def list_models(self, ctx):
        """Lists all available models from the API endpoint."""
        try:
            models = await self.get_available_models()
        except Exception as e:
            return await ctx.send(f"Error retrieving models: {e}")
        
        if not models:
            return await ctx.send("No models available.")
        
        # Erstelle eine einfache Liste der Modell-IDs (oder anderer relevanter Informationen)
        model_list = "\n".join(model["id"] for model in models if "id" in model)
        await ctx.send(f"Available models:\n```{model_list}```")

    @aihelpowner.command(name="setmodel")
    async def aihelpowner_setmodel(self, ctx, model: str):
        """
        Sets the default model to be used for AI requests.
        Example: `!aihelpowner setmodel gpt-3.5-turbo`
        """
        if not (await self.is_owner(ctx)):
            await ctx.send("You do not have permission to use this command.")
            return
    ...
    async def aihelphelp(self, ctx):
        ...
        help_text = (
            ...
            "| !aihelpconfig listsources [<category>]                        | Lists source files (Owner/Admin only)                      |\n"
            "| !aihelpconfig removesource <category> <filename>              | Removes a source file (Owner/Admin only)                   |\n"
            "| !aihelpconfig updatesourcetags <category> <filename> <tags>     | Updates the tags for a source (Owner/Admin only)           |\n"
            "| !aihelpconfig movesource <old_cat> <filename> <new_cat>         | Moves a source file to another category (Owner/Admin only)   |\n"
            "| !aihelpconfig setchannels <channel ids>                       | Sets proactive support channels (Owner/Admin only)         |\n"
            "| !aihelpconfig reloadsources                                   | Reloads all sources from disk (Owner/Admin only)           |\n"
            "| !aihelprate <category> <filename> <up|down>                   | Rates a source as helpful (Everyone)                       |\n"
            "| !aihelphelp                                                  | Displays this help information                             |\n"
            "+--------------------------------------------------------------+------------------------------------------------------------+\n"
            "```\n\n"
            "Replace `<url>` with your API endpoint (e.g. https://api.openai.com/v1/chat/completions).\n"
        )
